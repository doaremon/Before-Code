...
public class MySwitch extends CompoundButton {
	...
	private Layout makeLayout(CharSequence text) {
		return new StaticLayout(text, mTextPaint, (int) android.util.FloatMath.ceil(Layout
				.getDesiredWidth(text, mTextPaint)), Layout.Alignment.ALIGN_NORMAL, 1.f, 0, true);
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			case TOUCH_MODE_DRAGGING: {
				...
				if (mOrientation == HORIZONTAL) {
					float newPos = Math
							.max(0, Math.min(mThumbPosition + dx, getThumbScrollRange()));
				...
				if (mOrientation == VERTICAL) {
					float newPos = Math
							.max(0, Math.min(mThumbPosition + dy, getThumbScrollRange()));
					if (newPos != mThumbPosition) {
						mThumbPosition = newPos;
						mTouchY = y;
						invalidate();
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int switchWidth;
		if (mOrientation == HORIZONTAL) {
			switchWidth = Math.max(mSwitchMinWidth, maxTextWidth * 2 + mThumbTextPadding * 2
					+ mTrackTextPadding * 2 + mTrackPaddingRect.left + mTrackPaddingRect.right);
			if (mTextOnThumb == false) {
				switchWidth = Math.max(maxTextWidth + mThumbWidth + mTrackTextPadding * 2
						+ (mTrackPaddingRect.right + mTrackPaddingRect.left) / 2, mSwitchMinWidth);
			}

			if (this.mPushStyle) {
				switchWidth = Math.max(mSwitchMinWidth, maxTextWidth + mThumbWidth
						+ mTrackTextPadding + (mTrackPaddingRect.left + mTrackPaddingRect.right)
						/ 2);
			}
		} else {
			switchWidth = Math.max(maxTextWidth + mThumbTextPadding * 2 + mThPad.left
					+ mThPad.right, mThumbWidth);
			if ((this.mPushStyle) || (mTextOnThumb == false)) {
				switchWidth = Math.max(maxTextWidth + mTrackTextPadding * 2
						+ mTrackPaddingRect.left + mTrackPaddingRect.right, mThumbWidth);
			}
		}
		...
		if (mOrientation == VERTICAL) {
			...
			if (this.mPushStyle) {
				switchHeight = Math.max(mSwitchMinHeight, maxTextHeight + mThumbHeight
						+ mTrackTextPadding + (mTrackPaddingRect.top + mTrackPaddingRect.bottom)
						/ 2);
			}
		}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		Log.d(TAG, "getPaddingLeft()=" + getPaddingLeft() + " getPaddingRight()="
				+ getPaddingRight());
		Log.d(TAG, "getPaddingTop()=" + getPaddingTop() + " getPaddingBottom()="
				+ getPaddingBottom());
		...
		if (mPushStyle) {
			...
			final int maxTextWidth = Math.max(mOnLayout.getWidth(), mOffLayout.getWidth());
			final int maxTextHeight = Math.max(mOnLayout.getHeight(), mOffLayout.getHeight());
			int width = maxTextWidth * 2 + mTrackPaddingRect.left + mTrackPaddingRect.right
					+ mThumbWidth + mTrackTextPadding * 4;
			int height = mSwitchBottom - mSwitchTop;

			if (mOrientation == VERTICAL) {
				height = mTrackPaddingRect.top + mTrackTextPadding + maxTextHeight
						+ mTrackTextPadding + mThumbHeight + mTrackTextPadding + maxTextHeight
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mOrientation == VERTICAL) {
			...
			if (mPushStyle) {
				...
				backingLayer.translate(0, maxTextHeight + mTrackTextPadding * 2
						+ (maxTextHeight - mOnLayout.getHeight()) / 2 + mThumbHeight);// +
																						// mThPad.left
																						// +
																						// mThPad.right,)
				if (mDrawableOn != null)
		...
		if (mOrientation == HORIZONTAL) {
			...
			int dxOffText = mTextOnThumb ? (thumbL + thumbR) / 2 - mOffLayout.getWidth() / 2
					+ mTrackTextPadding - mThumbTextPadding // (thumbL+thumbR)/2
															// already has
															// 2*mThumbTextPadding
			...
			int thumbBoundL = switchInnerLeft + thumbPos - mThumbExtraMovement;// +
																				// mThPad.left
			int thumbBoundR = switchInnerLeft + thumbPos + mThumbWidth - mThumbExtraMovement;// -
																								// mThPad.right
