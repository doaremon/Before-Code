...
public class Switch extends Base {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		res_top_finish = R.string.control_switch;
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (position == jsonData.length()) {
				...
				try {
					...
					iv_icon.setImageResource(item.getInt("status") == 1 ? R.drawable.icon_control_switch_on : R.drawable.icon_control_switch_off);
				} catch (JSONException e) {
					e.printStackTrace();
				}
			}
