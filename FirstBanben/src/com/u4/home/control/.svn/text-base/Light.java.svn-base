...
public class Light extends Base {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		res_top_finish = R.string.control_light;
	...
	protected void onResume() {
		...
		gv_list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View v, int position, long arg3) {
				try {
					if (position == jsonData.length()) { // new equipment
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (position == jsonData.length()) {
				...
				try {
					...
					iv_icon.setImageResource(item.getInt("status") == 1 ? R.drawable.icon_control_light_on : R.drawable.icon_control_light_off);
				} catch (JSONException e) {
					e.printStackTrace();
				}
			}
